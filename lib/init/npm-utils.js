...
// td does not supports mocking indirect deps like proxyquire
// added a func to set fs to memfs(should only used for testing)
let _fs = fs;
...
function findPackageJson(startDir) {
    ...
    do {
        ...
        if (!_fs.existsSync(pkgFile) || !_fs.statSync(pkgFile).isFile()) {
            dir = path.join(dir, "..");
            continue;
        }
        return pkgFile;
...
function check(packages, opt) {
    ...
    try {
        fileJson = JSON.parse(_fs.readFileSync(pkgJson, "utf8"));
    } catch (e) {
