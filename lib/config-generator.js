...
const helperContent = `import { FlatCompat } from "@eslint/eslintrc";
...
export default defineConfig([\n${exportContent || "  {}\n"}]);\n`; // defaults to `[{}]` to avoid empty config warning
     ...
     */
    async output() {
        // Install latest version explicitly (If version scope is NOT specified)
        this.result.devDependencies = this.result.devDependencies.map(deps => /^(@([A-Z0-9\-])+\/)?([A-Z0-9\-])+@([A-Z0-9\-])+$/gi.test(deps) ? deps : `${deps}@latest`)

        log.info(`The config that you've selected requires the following dependenc${this.result.devDependencies.length > 1 ? 'ies' : 'y'}:\n`);
