...
export class ConfigGenerator {
    ...
    async prompt() {
        ...
        if (answers.purpose === "style") {
            ...
            const tsStyleGuides = [
                ...
                { message: "XO: https://github.com/xojs/eslint-config-xo-typescript", name: "xo", value: { packageName: ["eslint-config-xo", "eslint-config-xo-typescript"], type: "eslintrc" } }
            ];
    ...
    calc() {
        ...
        if (this.answers.purpose === "syntax") {
            ...
            this.result.devDependencies.push(this.answers.styleguide.packageName, "@eslint/js");

            // install peer dependencies - it's needed for most eslintrc-style shared configs.
            const peers = fetchPeerDependencies(this.answers.styleguide.packageName);

            if (peers !== null) {
                this.result.devDependencies.push(...peers);
            }

            if (this.answers.styleguide.type === "flat" || this.answers.styleguide.type === void 0) {
                importContent += `import styleGuide from "${this.answers.styleguide.packageName}";\n`;
                exportContent += "  styleGuide,\n";
            } else if (this.answers.styleguide.type === "eslintrc") {
                ...
                const extended = Array.isArray(this.answers.styleguide.packageName)
                    ? this.answers.styleguide.packageName
                    : [this.answers.styleguide.packageName];
                const extendedNames = extended.map(it => it.replace("eslint-config-", ""));

                exportContent += `  compat.extends("${extendedNames.join(", ")}"),\n`;
            }
        }
