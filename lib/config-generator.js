...
export class ConfigGenerator {
	 ...
	 * @param {string} [options.packageJsonPath] The path to package.json.
	 * @param {Object} [options.answers] The answers provided by the user.
	 */
	constructor(options) {
		this.cwd = options.cwd;
		this.packageJsonPath =
			options.packageJsonPath || findPackageJson(this.cwd);
	...
	async prompt() {
		Object.assign(
			this.answers,
			await promptQuestions(langQuestions, this.answers),
		);

		if (this.answers.languages.includes("javascript")) {
			Object.assign(
				this.answers,
				await promptQuestions(jsQuestions, this.answers),
			);
		}
		...
		if (this.answers.languages.includes("md")) {
			Object.assign(
				this.answers,
				await promptQuestions(mdQuestions, this.answers),
			);
		}
		...
		if (this.answers.configFileLanguage === "ts") {
			...
			if (semverLessThan(nodeVersion, "24.3.0")) {
				...
				Object.assign(
					this.answers,
					await promptQuestions(addJitiQuestion, this.answers),
				);
			}
		}
	}
	...
	async calc() {
		...
		this.answers.config =
			typeof this.answers.config === "string"
		...
		if (languages.includes("javascript")) {
			...
			if (
				this.answers.moduleType === "commonjs" ||
				this.answers.moduleType === "script"
			) {
		...
		if (this.answers.config) {
			...
			const eslintIndex = peers.findIndex(dep =>
			...
			if (eslintIndex === -1) {
				...
				isDefineConfigExported = !semverGreaterThanRange(
					"9.22.0",
					versionRequirement,
				);
			...
			if (config.type === "flat" || config.type === void 0) {
				...
				const shorthandName = getShorthandName(
					packageName,
					"eslint-config",
				);
...
export default defineConfig([\n${exportContent || "  {}\n"}]);\n`; // defaults to `[{}]` to avoid empty config warning
	...
	async output() {
		...
		log.message(this.result.devDependencies.join(", "));

		const { executeInstallation, packageManager } = await promptQuestions(
			installationQuestions,
			this.answers,
		);
		...
		if (executeInstallation === true) {
			...
			const result = spawnSync(
				process.execPath,
				[eslintBin, "--fix", "--quiet", configPath],
				{ encoding: "utf8" },
			);
