 ...
 * @throws {Error} If the package is not found or if there is an error fetching the package.
 * @returns {Promise<string[]>} Gotten peerDependencies. Returns null if npm was not found.
 */
async function fetchPeerDependencies(packageName) {
	...
	try {
		...
		throw new Error(
			`Cannot fetch "${name}@${version}" with error: ${err.message || err}`,
		);
	}
}
 ...
 * @param {string} [opt.startDir] Directory to begin searching from
