 ...
 * @param {Array} questionsPromptArray Array of questions prompt.
 * @returns {Array} Questions prompt with style options.
 */
function setQuestionsPromptStyle(questionsPromptArray) {
    return questionsPromptArray.map(opts => ({
        ...
        symbols: {
            prefix(state) {
                return ({
                    pending: color.cyan(color.symbols.question),
                    cancelled: color.red(color.symbols.cross),
                    submitted: color.green(color.symbols.check)
                })[state.status];
            },
            indicator: {
                on: color.cyan(color.symbols.radioOn),
                off: color.gray(color.symbols.radioOff)
            }
        }
    }));
}
